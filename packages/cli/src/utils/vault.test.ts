...
describe("vault utilities", () => {
  describe("getVaultOption", () => {
    let getVaultOption: (cmd: Command) => string | undefined;

    beforeAll(async () => {
      const module = await import("./vault.js");
      getVaultOption = module.getVaultOption;
    });

    it("should return undefined when no vault option is set", () => {
    ...
    it("should return local vault option when set via setOptionValue", () => {
      const cmd = new Command();
      cmd.option("--vault <path>", "Vault path");
    ...
    it("should return parent vault option when local is not set", () => {
